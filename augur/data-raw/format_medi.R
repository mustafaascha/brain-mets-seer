...
.init_format_medi <- function(sas_input_file) {                                 
  #Note: "char_count" and "width" are equivalent, each describing the number of characters of a variable
  ...
  vars_that_should_be_expanded <- which(grepl("\\(", to_return$vars))
  if(length(vars_that_should_be_expanded) > 0){
    vtsbe <- vars_that_should_be_expanded
    new_widths <-with(to_return,
...
#' 
#' 
.parse_nch <- function(si_file) {
  to_return <- suppressWarnings(.init_format_medi(si_file))
...
#' Parse the outpatient infile
#' 
#' @param sas_input_file The SAS file describing a FWF claims file.
#' @param formatfile.type The SEER-Medicare infile type being formatted.
#' 
#' 
.parse_outs <- function(si_file) {
  outsaf_format <- suppressWarnings(.init_format_medi(si_file))
...
format_medi <- function(sas_input_file, formatfile.type = "default"){ 
  ...
  if(formatfile.type %in% easy_to_parse) {
    ...
    #special parser
    to_return <- .parse_nch(sas_input_file)
  } else if(formatfile.type == "outsaf" |
          formatfile.type == "outpat") {
    #special parser
    to_return <- .parse_outs(sas_input_file)
